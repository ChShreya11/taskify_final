...
function getMonthlyDates(pickedFirstStartDate, endDate) {
  const startMoment = moment(pickedFirstStartDate, "DD/MM/YYYY");
  ...
  while (!startMoment.isAfter(endMoment)) {
    const lastDayOfMonth = moment(
      startMoment.endOf("month").format("DD-MM-YYYY"),
      "DD/MM/YYYY"
    );
    console.log(startMoment);
    console.log(lastDayOfMonth);
    monthlyDates.push(
      startMoment.isSameOrBefore(lastDayOfMonth) ? startMoment : lastDayOfMonth
    );
    ...
    case "daily":
      ...
      while (date <= lastDate) {
        const formattedDate = date
          ...
          .join("/");
        const temp = moment(formattedDate, "MM/DD/YYYY").format("DD/MM/YYYY");

        progressEachDay[temp] = 0;
